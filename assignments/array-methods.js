...
runners.forEach((element, index) => {
  fullName.push(`${element.first_name} ${index.last_name}`);
});
...
let largeShirts = runners.filter(state => state.shirt_size === "L");

console.log(largeShirts);

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations into a ticketPriceTotal array and log the result
let ticketPriceTotal = runners.reduce((total, state) => {
...
// Problem 1
let allLowerCase = runners.map(state => {
  return state.last_name.toLowerCase();
});
...
let runnerName = runners.filter(state => state.first_name.length > 7);

console.log(runnerName);

// Problem 3
let ticketMedianTotal = runners.length;
let population = runners.reduce((total, currentValue) => {
  return (total += currentValue.donation);
}, 0);
console.log(population / 49);
